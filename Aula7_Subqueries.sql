-- Traz os vendedor que já realizaram pedidos
SELECT v.nome_vendedor, v.codigo_vendedor FROM vendedor v
	WHERE v.codigo_vendedor IN
		(SELECT p.codigo_vendedor FROM pedido p)
	ORDER BY v.nome_vendedor;

-- Funciona mas traz várias ocorrências de vendedores que venderam mais de uma vez
SELECT v.nome_vendedor, v.codigo_vendedor FROM vendedor v
	INNER JOIN pedido p
	ON v.codigo_vendedor = v.codigo_vendedor
	ORDER BY v.nome_vendedor;

SELECT DISTINCT v.nome_vendedor, v.codigo_vendedor FROM vendedor v
	INNER JOIN pedido p
	ON v.codigo_vendedor = v.codigo_vendedor
	ORDER BY v.nome_vendedor;

-- Não é possível fazer essa consulta sem utilizar subconsutla
SELECT v.nome_vendedor, v.codigo_vendedor, min(p.prazo_entrega) FROM vendedor v
	INNER JOIN pedido p 
	ON v.codigo_vendedor = p.codigo_vendedor
	GROUP BY v.nome_vendedor, v.codigo_vendedor;

SELECT v.nome_vendedor, v.codigo_vendedor, p.prazo_entrega FROM vendedor v
	INNER JOIN pedido p 
	ON v.codigo_vendedor = p.codigo_vendedor
	WHERE prazo_entrega IN (SELECT min(prazo_entrega) FROM pedido);

SELECT min(prazo_entrega) FROM pedido;

-- Traz os vendedor que não realizaram pedidos
SELECT v.nome_vendedor FROM vendedor v
	WHERE v.codigo_vendedor NOT IN
		(SELECT p.codigo_vendedor FROM pedido p);
...
SELECT c.uf FROM pedido p
	INNER JOIN cliente c
	ON p.codigo_cliente = c.codigo_cliente
	WHERE c.uf = 'SP';

SELECT * FROM pedido p
		WHERE p.codigo_vendedor IN
			(SELECT v.codigo_vendedor, v.salario_fixo FROM vendedor v
				WHERE v.salario_fixo > 10000
			);

SELECT * FROM empregado ORDER BY salario DESC;

-- Consulta que retorne o nome e o salario dos empregados que possuem o maior salario
SELECT e.pnome, e.salario FROM empregado e
	WHERE salario IN 
		(SELECT max(salario) FROM empregado);

SELECT e.cargo, e.pnome, e.salario FROM empregado e
	WHERE (e.cargo, e.salario) IN 
		(SELECT e.cargo, max(salario) FROM empregado e
		    GROUP BY e.cargo)
...
--Versão do banco de dados: 20180312
...
insert into versao values (default, '20180312', '12/03/2018');
