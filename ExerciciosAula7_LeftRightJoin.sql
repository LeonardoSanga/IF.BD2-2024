...
SELECT C.nome_cliente, p.prazo_entrega FROM cliente C
	...
	ORDER BY p.prazo_entrega;

SELECT C.nome_cliente, V.nome_vendedor, p.prazo_entrega FROM cliente C
	INNER JOIN pedido P
	ON C.codigo_cliente = P.codigo_cliente
...
SELECT p.descricao, p.unidade, ip.quantidade FROM produto p
	LEFT JOIN item_pedido ip
	ON p.codigo_produto = ip.codigo_produto;

-- mostra os pedidos que não se relacionaram com item_pedido
SELECT p.descricao, p.unidade, ip.quantidade FROM produto p
	LEFT JOIN item_pedido ip
	ON p.codigo_produto = ip.codigo_produto
...
SELECT p.descricao, p.unidade, ip.quantidade FROM produto p
	RIGHT OUTER JOIN item_pedido ip
	ON p.codigo_produto = ip.codigo_produto;

SELECT p.descricao, p.unidade, ip.quantidade FROM item_pedido ip
...
SELECT v.nome_vendedor, p.prazo_entrega FROM pedido p
	RIGHT OUTER JOIN vendedor v
	ON p.codigo_vendedor = v.codigo_vendedor;

SELECT * FROM vendedor;
INSERT INTO vendedor VALUES (350, 'Pedro Antonio', 3333, 'B');

SELECT v.nome_vendedor, c.nome_cliente, p.prazo_entrega FROM vendedor v
	LEFT OUTER JOIN pedido p
	ON v.codigo_vendedor = p.codigo_vendedor
	FULL OUTER JOIN cliente c
	ON p.codigo_cliente = c.codigo_cliente;

SELECT v.nome_vendedor, c.nome_cliente, p.prazo_entrega FROM pedido p
	FULL OUTER JOIN vendedor v
	ON p.codigo_vendedor = v.codigo_vendedor
	FULL OUTER JOIN cliente c
	ON p.codigo_cliente = c.codigo_cliente;
